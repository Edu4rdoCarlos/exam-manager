...
└── modules/
    ├── auth/            ← login, JWT issuance — no user CRUD
    ├── users/           ← user registration and management — no token logic
    ├── students/
    └── exams/
        ├── exam/
        ├── versions/
...
### auth vs users boundary

| Concern | Module |
|---|---|
| Login (verify credentials, issue JWT) | `auth` |
| Register user, get user profile | `users` |
| Password hashing | `users/infrastructure` (at persistence time) |
| Token verification / guards | `auth` (shared guard) |

`auth` depends on `users` data indirectly via its own `AuthUserRepository` port — it reads `{ id, email, passwordHash }` without coupling to the `User` domain entity. Deleting `users` module must not require changing `auth` beyond removing the infrastructure adapter.
